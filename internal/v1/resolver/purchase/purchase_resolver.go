...
func (r *resolver) Created(trx *gorm.DB, input *purchaseModel.Created) interface{} {
	...
	trx.Commit()
	return code.GetCodeMessage(code.Successful, purchase.Purchase_ID)
}

func (r *resolver) List(input *purchaseModel.Fields) interface{} {
	input.Is_Deleted = util.PointerBool(false)
...
func (r *resolver) Deleted(input *purchaseModel.Updated) interface{} {
	_, err := r.PurchaseService.GetByID(&purchaseModel.Field{Purchase_ID: input.Purchase_ID,
		Is_Deleted: util.PointerBool(false)})
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (r *resolver) Updated(input *purchaseModel.Updated) interface{} {
	purchase, err := r.PurchaseService.GetByID(&purchaseModel.Field{Purchase_ID: input.Purchase_ID,
		Is_Deleted: util.PointerBool(false)})
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
