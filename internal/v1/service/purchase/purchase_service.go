...
func (s *service) Created(input *model.Created) (output *model.Base, err error) {
	...
	output.Purchase_ID = util.GenerateUUID()
	output.Created_At = util.NowToUTC()
	output.Is_Deleted = false
...
func (s *service) Deleted(input *model.Updated) (err error) {
	field, err := s.Entity.GetByID(&model.Field{Purchase_ID: input.Purchase_ID,
	...
	field.Updated_By = input.Updated_By
	field.Updated_At = util.PointerTime(util.NowToUTC())
